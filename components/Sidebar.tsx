    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch("https://formsubmit.co/ajax/josuesegatofilho@gmail.com", {
                ...
                body: JSON.stringify({ _subject: "Sidebar Contact Form Submission", ...formData })
            });
            if (response.ok) {
                setIsSuccess(true);
        ...
        <div className="flex flex-col gap-10">
            ...
            <section>
                <div className="flex items-center gap-2 mb-5 text-brand-yellow">
                    ...
                    <h3 className="text-[11px] font-bold uppercase tracking-[0.2em]">{t('sidebar.connect_title')}</h3>
                </div>
                <div className="grid grid-cols-4 gap-2">
                    {[
                        ...
                        <a key={i} href={social.href} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center h-10 rounded-xl bg-white/[0.03] border border-white/10 text-brand-textTertiary hover:border-brand-yellow/30 hover:text-brand-yellow hover:bg-brand-yellow/5 transition-all duration-300 group" aria-label={social.label}>
            ...
            <section>
                <div className="flex items-center gap-2 mb-5 text-brand-yellow">
                    ...
                    <h3 className="text-[11px] font-bold uppercase tracking-[0.2em]">{t('sidebar.contact_title')}</h3>
                </div>
                <form className="space-y-3" onSubmit={handleSubmit}>
                    <div className="space-y-2">
                        <input
                            type="text" required
                            placeholder={t('sidebar.labels.name')}
                            value={formData.name}
                        ...
                        <input
                            type="email" required
                            placeholder={t('sidebar.labels.email')}
                        ...
                        <textarea
                            rows={3} required
                            placeholder={t('sidebar.labels.message')}
